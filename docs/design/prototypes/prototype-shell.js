...
;(() => {
  ...
  const link = (href, name, label, current = false) =>
    `<a href="${href}" title="${label}" aria-label="${label}" ${current ? 'aria-current="page"' : ""}>${icon(name)}</a>`
  if (page === "standalone") {
    shell.innerHTML =
      '<a class="prototype-back" href="inspira-library.html"><span>←</span>返回 Library</a>'
    const route = () => {
      const settings = location.hash === "#settings"
      document.getElementById("settingsPage").hidden = !settings
      document.getElementById("insightsPage").hidden = settings
      document.title = `Inspira — ${settings ? "Settings" : "Insights"}`
      window.scrollTo(0, 0)
    }
  ...
  let mode = "light"
  try {
    mode = localStorage.getItem("inspira-prototype-theme") || "light"
  } catch {}
  function setTheme(value) {
    mode = value
    document.documentElement.dataset.theme = value
    try {
      localStorage.setItem("inspira-prototype-theme", value)
    } catch {}
    document
      .querySelectorAll(".theme-option")
      .forEach((el, i) =>
        el.classList.toggle("active", (value === "dark" ? 1 : 0) === i)
      )
    document
      .getElementById("themeToggle")
      ?.setAttribute("aria-pressed", String(value === "dark"))
  }
  setTheme(mode)
  document
    .getElementById("themeToggle")
  ...
  document.querySelectorAll(".settings-nav-item").forEach(
    (el, i) =>
      (el.onclick = () => {
        document
        ...
        document
          .querySelectorAll(".settings-group")
          [i].scrollIntoView({ behavior: "smooth", block: "start" })
      })
  )
  const dialog = document.createElement("dialog")
  dialog.id = "prototypeDetail"
  dialog.className = "prototype-detail"
  dialog.setAttribute("aria-labelledby", "prototypeDetailTitle")
  dialog.innerHTML =
    '<div class="detail-layout"><div class="detail-preview"></div><section class="detail-sidebar"><h2 id="prototypeDetailTitle"></h2><p class="detail-date">保存于 2026年9月6日</p><div class="detail-label">TAGS</div><div class="prototype-tags"></div><form class="prototype-tag-form"><input aria-label="添加标签" placeholder="添加标签…" maxlength="60"><button aria-label="添加标签" type="submit">+</button></form><label for="prototypeNotes">NOTES</label><textarea id="prototypeNotes" placeholder="记下你被打动的地方…"></textarea></section></div><button class="prototype-detail-close" aria-label="关闭详情" autofocus>×</button>'
  document.body.append(dialog)
  let selected, previousOverflow
  function renderTags() {
    const tags = dialog.querySelector(".prototype-tags")
    tags.replaceChildren()
    ;(selected.tags || []).forEach((tag, i) => {
      const chip = document.createElement("span")
      chip.className = "prototype-tag"
      chip.append(document.createTextNode(tag))
      const remove = document.createElement("button")
      remove.textContent = "×"
      remove.setAttribute("aria-label", `移除标签 ${tag}`)
      remove.onclick = () => {
        selected.tags.splice(i, 1)
        renderTags()
      }
      chip.append(remove)
      tags.append(chip)
  ...
  window.showPrototypeDetail = (data) => {
    selected = data
    const preview = dialog.querySelector(".detail-preview")
    preview.replaceChildren()
    dialog.querySelector("h2").textContent = data.title || "灵感"
    if (data.img) {
      const img = document.createElement("img")
      img.src = data.img
      img.alt = data.title || "灵感图片"
      preview.append(img)
    }
    if (data.text) {
    ...
    renderTags()
    dialog.querySelector("textarea").value = data.notes || ""
  ...
  dialog.querySelector("textarea").oninput = (e) =>
    (selected.notes = e.target.value)
  dialog.querySelector("form").onsubmit = (e) => {
    e.preventDefault()
    const input = dialog.querySelector("input")
    if (input.value.trim()) {
      ;(selected.tags ??= []).push(input.value.trim())
      input.value = ""
      renderTags()
    }
  }
  const cardData = new WeakMap()
  window.bindPrototypeCards = (root = document) =>
    root.querySelectorAll(".card").forEach((card) => {
      card.tabIndex = 0
      card.setAttribute("role", "button")
      const img = card.querySelector("img")
      const title =
        card.querySelector(".card-web-title,.card-video-title")?.textContent ||
        img?.alt ||
        "文字灵感"
      card.setAttribute("aria-label", `查看 ${title}`)
      if (!cardData.has(card))
        cardData.set(card, {
          title,
          img: img?.src,
          text: img ? undefined : card.innerText,
          tags: ["灵感"],
          notes: ""
        })
      const open = () => {
        ...
        if (card.classList.contains("card-video")) {
          data.text =
            "视频链接示例：日本侘寂美学的当代诠释。当前原型未绑定可播放视频。"
        }
        window.showPrototypeDetail(data)
      }
      card.onclick = open
  ...
  document.querySelector(".prototype-add")?.addEventListener("click", () => {
    ...
    modal.className = "prototype-small-dialog"
    modal.innerHTML =
